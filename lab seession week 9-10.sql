-- 1
create schema orderg1;
use orderg1;
 -- Supplier (SUPP_ID,SUPP_NAME,SUPP_CITY,SUPP_CITY)
 DROP tables IF exists  SUPPLIER,CUSTOMER;
...
select cus_id, cus_name , cus_gender from customer where customer.cus_name like 'A%' OR customer.cus_name like '%A';

 -- -- 9)	Create a stored procedure to display the Rating for a Supplier if any along with the Verdict
--  on that rating if any like if rating >4 then “Genuine Supplier” if rating >2 “Average 
